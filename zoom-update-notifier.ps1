...
This email was generated by $(Split-Path -Path $PSCommandPath -Leaf) at $((Get-Date).ToString('dd/MM/yyyy HH:mm:ss')) on $(([System.Net.Dns]::GetHostEntry(($env:computerName))).Hostname).
"@
  Send-MailMessage -SmtpServer $MailServer -From $MailFromAddress -To $($MailToAddress.Split(',') | ForEach-Object { $_.Trim() }) -Subject $MailSubject -Body $emailBody
}
...
[version]$installedVersion = Get-ZoomExeVersion -Path $Script:ZoomExePath
[version]$latestVersion = Get-ZoomLatestVersion
